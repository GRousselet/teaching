...
```{r}
# dice / type of participants available
dice <- c(4, 6, 8, 10, 12, 20)
# weights / proportions of each type for population 1
pop1 <- c(0, 5, 6, 3, 1, 0)
pop2 <- c(2, 4, 3, 4, 2, 0)
pop3 <- c(0, 3, 4, 4, 2, 2)
```
...
for(P in 1:np){
  res1[,P] <- runif(nt, min = 0, max = samp1[P])
  res2[,P] <- runif(nt, min = 0, max = samp2[P])
  res3[,P] <- runif(nt, min = 0, max = samp3[P])
}
```
...
for(sim in 1:nsim){
  ...
  # sample trials for each participant
  for(P in 1:np){
    res1[,P] <- runif(nt, min = 0, max = samp1[sim,P])
    res2[,P] <- runif(nt, min = 0, max = samp2[sim,P])
    res3[,P] <- runif(nt, min = 0, max = samp3[sim,P])
  }
